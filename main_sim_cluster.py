...
if (args.testOnly):
    ...
    checkpoint = torch.load('./checkpoint/'+'cifar100'+os.sep+file_name+'_similarity_regularized_layerwiseRegStrength_lambda20.0_eta0.02_cluster.t7')
    net = checkpoint['net']
...
#matrices_rob = torch.load("robust_matrices.pt")
...
def train(epoch):
    ...
    correct = 0
    total = 0
    if(args.resume and start_epoch==200):
    ...
    for batch_idx, (inputs_c, targets_c) in enumerate(loader):
        ...
        if(sim_learning):
            ...
            for matrix in matrices_rob:
                matrix[torch.sqrt(1-matrix) !=torch.sqrt(1-matrix)] = 0
            clusters = [AgglomerativeClustering(n_clusters=12).fit(torch.sqrt(1-matrices_rob[i].detach().cpu())) for i in range(0, len(matrices_rob))]
            ...
            for i, (r, g) in enumerate(zip(matrices_reg, matrices_rob)):
                #set_trace()
                sim_loss = get_sim_loss(i, r[indices[i]][:,indices[i]],g[indices[i]][:,indices[i]], 1e-4)
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir(save_point):
            os.mkdir(save_point)
        if(sim_learning):
            torch.save(state, save_point+file_name+'_similarity_regularized_layerwiseRegStrength_lambda'+str(args.lamb)+'_eta'+str(args.eta)+'_cluster.t7')
...
if not os.path.isdir(save_point):
    os.mkdir(save_point)
if(sim_learning):
   torch.save(state, save_point+file_name+'_similarity_regularized_layerwiseRegStrength_lambda'+str(args.lamb)+'_eta'+str(args.eta)+'cluster_end.t7')
